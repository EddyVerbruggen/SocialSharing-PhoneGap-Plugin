...
- (void)shareViaInternal:(CDVInvokedUrlCommand *)command
    ...
    for (NSString *filename in filenames) {
        ShareContent *content= [self getShareContent:filename];
        if(content!=nil){
            if([content getType]==IMAGE)
                [composeViewController addImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[content getUrl]]]];
            else if([content getType]==VIDEO)
                [composeViewController addVideoURL:[content getUrl]];
        }
    }
...
- (void)shareViaWhatsApp:(CDVInvokedUrlCommand *)command {
    ...
    // with WhatsApp, we can share an content (image, video, etc) OR text+url.. content wins if set
    if (content != nil) {
...
- (NSObject *)getMedia:(NSString *)fileName {
    UIImage *image = [self getImage:fileName];
    if (image != nil)
        return image;
...
- (ShareContent *)getShareContent:(NSString *)fileName {
    ShareContent *shareContent = [[ShareContent alloc] init];
    NSURL *file = [self getFile:fileName];
    [shareContent setUrl:file];
    return shareContent;
}
